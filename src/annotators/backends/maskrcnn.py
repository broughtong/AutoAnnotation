...
class AnnotatorBackend():
    ...
    def __init__(self, path, scanField, annotationFolder, annotationField):
        ...
        self.folderName = os.path.join(path, scanField, self.annotationFolder + "_" + annotationField, "maskrcnn", "all")
        os.makedirs(os.path.join(self.folderName, "imgs"), exist_ok=True)
        os.makedirs(os.path.join(self.folderName, "masks"), exist_ok=True)
        os.makedirs(os.path.join(self.folderName, "debug"), exist_ok=True)

    def annotate(self, filename, pc, annotations):
        
        #pc only
        fn = os.path.join(self.folderName, "imgs", filename + ".png")
        draw.drawImgFromPoints(fn, pc, [], [], [], [], dilation=3)

        #masks
        fn = os.path.join(self.folderName, "masks", filename + ".png")
        carPoints, nonCarPoints = geometry.getInAnnotation(pc, annotations)
        badAnnotation = self.drawAnnotation(fn, annotations)

        #debug
        fn = os.path.join(self.folderName, "debug", filename + ".png")
        draw.drawImgFromPoints(fn, pc, [], [], annotations, [], dilation=None)
